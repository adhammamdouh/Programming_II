...
VendingMachine::VendingMachine(char ProductName[10][30], double Price[10]) // Constructor
{
	Money = new PayAndReminder(0, 0.0);			// new allocate the PayAndRemainder Object
	for (int i = 0; i < 10; ++i)				//loop for setting the item's name, price and the amount
...
void VendingMachine::Menu()
{
	...
	int choose;									// "choose" a variable to select the item needed
	cout << "Welcome" << endl;

	while (true)
	{
		short choice;                           // "choice" is a variable used to select a choice among choices
...
{
	int * choose = &choice;									//Choose a pointer to int that point to Choose int the last Function
	int which;						// Which is a variable to select between to choices
